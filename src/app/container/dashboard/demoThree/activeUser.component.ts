      ...
      <nz-table [nzData]="seller[tab.key]" [nzFrontPagination]="false" [nzShowPagination]="false" class="text-sm rounded- [5px]">
        ...
        <tbody class="bg-white dark:bg-[#1b1d2a]">
          <tr *ngFor="let product of seller[tab.key]" class="group">
            ...
            <td class="px-4 py-2.5 font-normal last:text-end capitalize text-[14px] text-dark dark:text-white/[.87] border-none group-hover:bg-transparent">{{ product.company }}</td>
            <td class="px-4 py-2.5 font-normal last:text-end capitalize text-[14px] text-dark dark:text-white/[.87] border-none group-hover:bg-transparent">{{ product.product }}</td>
            <td class="px-4 py-2.5 font-normal last:text-end capitalize text-[14px] text-dark dark:text-white/[.87] border-none group-hover:bg-transparent">{{ product.revenue }}</td>
            ...
            <span class="inline text-[13px] px-[11.85px] py-[4.5px] font-medium capitalize rounded-[15px] bg-{{ product.statusColor }}/10 text-{{ product.statusColor }}">{{ product.status }}</span>
            </td>
          </tr>
        </tbody>
      </nz-table>
...
export class ActiveUserComponent  {
  ...
  constructor() {
    this.seller = tableData.activeUserData;
    this.tabData = [
