      ...
      >
        ...
        >
          <div class="flex justify-between">
            ...
            <div
              class="absolute top-0 ltr:-right-[140px] rtl:-left-[140px] w-[230px] max-md:w-[210px] max-ssm:w-[230px] px-[30px] rounded-full h-full flex items-center justify-start overflow-hidden bg-{{
                overviewData.type
              }}/10 text-{{ overviewData.type }} "
            >
              <div
                class="flex items-center justify-center text-{{
                  overviewData.type
                }}"
              >
      ...
      >
        ...
        >
          <div class="flex justify-between">
            <div>
              <span class="font-normal text-body dark:text-white/60 text-15">
                <span *ngIf="balance.code == 'TL'">TL</span>
                <span *ngIf="balance.code == 'USD'">USD</span>
                <span *ngIf="balance.code == 'EUR'">EUR</span>
                Bakiye</span
              >
              <h4
                class="mb-0 text-3xl max-lg:text-[26px] max-sm:text-2xl font-semibold leading-normal text-dark dark:text-white/[.87]"
              >
                <span>
                  <span *ngIf="balance.code == 'TL'">₺</span>
                  <span *ngIf="balance.code == 'USD'">$</span>
                  <span *ngIf="balance.code == 'EUR'">€</span>
                  {{ balance.credit | number : '1.2-2' }}
                </span>
              </h4>
            </div>
            <div
              class="absolute top-0 ltr:-right-[140px] rtl:-left-[140px] w-[230px] max-md:w-[210px] max-ssm:w-[230px] px-[30px] rounded-full h-full flex items-center justify-start overflow-hidden bg-success/10 text-success "
            >
              <div class="flex items-center justify-center text-success">
                <div class="fill-success flex items-center">
                  <svg-icon
                    class="w-[35px] h-[35px] [&>svg]:w-[35px] [&>svg]:h-[35px] max-md:[&>svg]:w-[25px] max-md:[&>svg]:h-[25px]"
                    src="../../../../assets/images/svg/unicons-line/moneybag.svg"
                  ></svg-icon>
                </div>
              </div>
            </div>
...
export class OverviewListComponent implements OnInit {
  ...
  constructor(
    ...
    const number = this.appOverviewData.reduce(
  ...
  async ngOnInit(): Promise<void> {
    let balance = await this.companyService.getBalance(
